...
/**
 * This class is responsible for notifying others when the connection is lost or
 * established and for attempting to reconnect when the connection is lost.
 * 
 */
	...
	public void disconnected(ConnectionStatusListener noticer) {
		if (connected.compareAndSet(true, false)) {
			for (ConnectionStatusListener listener : listeners) {
				// The thread running the noticer is our current execution
				// thread. If we notify him he'll block and we'll be deadlocked. Since he
				// noticed the disconnect he is responsible for initiating his own wait state.
				if (listener == noticer)
					continue;
